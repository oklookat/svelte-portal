...
import TestPortalWrapper from './test_portal_wrapper.svelte';
import TestLifecycle from './test_lifecycle.svelte';
...
describe('<Portal /> target', () => {
	...
	beforeEach(() => {
		const { container } = render(TestPortalWrapper);
		wrapper = container;
	});

	it('should be rendererd in a specific HTML element', () => {
		const renderedDivInFooter = wrapper.querySelector('footer #renderedInFooter');

		expect(renderedDivInFooter).not.toBe(null);
	});

	it('should be rendererd in a specific HTML element with id', () => {
		const renderedDivInTargetId = wrapper.querySelector('#target #renderedInTarget');

		expect(renderedDivInTargetId).not.toBe(null);
	});

	it('should be rendererd in a specific HTML element with class', () => {
		const renderedDivInTargetClass = wrapper.querySelector('.target #renderedInTargetClass');
	...
	it('should not render a Portal at the origin', () => {
		...
		const isPortalContainerEmpty = portalContainer.innerHTML.trim().length === 0;
...
describe('<Portal /> lifecycle', () => {
	...
	beforeEach(() => {
		const { container, component } = render(TestLifecycle);
